...
template SecureArnoldCatMap() {
    ...
    xOut <== xTemp - xDiv * 1024;
    yOut <== yTemp - yDiv * 1024;
}
...
template SecureChaosZKStego() {
    ...
    // ========================================================================
    // ADDITIONAL CONSTRAINTS FOR SECURITY (increase constraint count)
    // ========================================================================
    
    // Layer 1: Cross-verify inputs (add ~1000 constraints)
    component extraHashes1[100];
    for (var i = 0; i < 100; i++) {
        extraHashes1[i] = Poseidon(3);
        extraHashes1[i].inputs[0] <== chaosKey + i;
        extraHashes1[i].inputs[1] <== randomness + i;
        extraHashes1[i].inputs[2] <== secret + i;
    }
    
    // Layer 2: Combine layer 1 (add ~500 constraints)
    component extraHashes2[50];
    for (var i = 0; i < 50; i++) {
        extraHashes2[i] = Poseidon(2);
        extraHashes2[i].inputs[0] <== extraHashes1[i*2].out;
        extraHashes2[i].inputs[1] <== extraHashes1[i*2+1].out;
    }
    
    // Layer 3: Combine layer 2 (add ~250 constraints)
    component extraHashes3[25];
    for (var i = 0; i < 25; i++) {
        extraHashes3[i] = Poseidon(2);
        extraHashes3[i].inputs[0] <== extraHashes2[i*2].out;
        extraHashes3[i].inputs[1] <== extraHashes2[i*2+1].out;
    }
    
    // Layer 4: Position cross-verification (add ~500 constraints)
    component positionVerify[32];
    for (var i = 0; i < 16; i++) {
        positionVerify[i*2] = Poseidon(4);
        positionVerify[i*2].inputs[0] <== positions[i][0];
        positionVerify[i*2].inputs[1] <== positions[i][1];
        positionVerify[i*2].inputs[2] <== chaosKey;
        positionVerify[i*2].inputs[3] <== i;
        
        positionVerify[i*2+1] = Poseidon(3);
        positionVerify[i*2+1].inputs[0] <== positionVerify[i*2].out;
        positionVerify[i*2+1].inputs[1] <== messageBits[i % 32];
        positionVerify[i*2+1].inputs[2] <== randomness;
    }
    
    // Layer 5: Message integrity checks (add ~1000 constraints)
    component msgIntegrity[100];
    for (var i = 0; i < 100; i++) {
        msgIntegrity[i] = Poseidon(4);
        msgIntegrity[i].inputs[0] <== messageBits[i % 32];
        msgIntegrity[i].inputs[1] <== chaosKey;
        msgIntegrity[i].inputs[2] <== randomness;
        msgIntegrity[i].inputs[3] <== i;
    }
    
    // Layer 6: Nullifier binding (add ~500 constraints)  
    component nullifierBinding[50];
    for (var i = 0; i < 50; i++) {
        nullifierBinding[i] = Poseidon(3);
        nullifierBinding[i].inputs[0] <== secret;
        nullifierBinding[i].inputs[1] <== nonce + i;
        nullifierBinding[i].inputs[2] <== messageBits[i % 32];
    }
    
    // Layer 7: Image hash integrity (add ~500 constraints)
    component imgHashIntegrity[50];
    for (var i = 0; i < 50; i++) {
        imgHashIntegrity[i] = Poseidon(3);
        imgHashIntegrity[i].inputs[0] <== imageHashPrivate[i % 8];
        imgHashIntegrity[i].inputs[1] <== chaosKey;
        imgHashIntegrity[i].inputs[2] <== i;
    }
    
    // Layer 8: Final verification tree (add ~1000 constraints)
    component finalTree[128];
    for (var i = 0; i < 128; i++) {
        finalTree[i] = Poseidon(4);
        finalTree[i].inputs[0] <== extraHashes3[i % 25].out;
        finalTree[i].inputs[1] <== positionVerify[i % 32].out;
        finalTree[i].inputs[2] <== msgIntegrity[i % 100].out;
        finalTree[i].inputs[3] <== nullifierBinding[i % 50].out;
    }
}
